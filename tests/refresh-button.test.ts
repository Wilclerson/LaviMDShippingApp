...
describe('placement', () => {
  test('the dashboard asks for the button', () => {
    assert.ok(read('src/app/page.tsx').includes('showRefresh'), 'dashboard opts in');
  });

  test('it sits with the Last sync indicator and is permission-gated', () => {
    const header = read('src/components/AppHeader.tsx');
    assert.ok(header.includes('Last sync:'), 'the indicator is here');
    assert.ok(header.includes('<RefreshDataButton />'));
    assert.ok(
      header.includes("can(user.role, 'sync:trigger')"),
      'the button is not rendered to a role that cannot use it',
    );
  });

  test('other pages do not render it', () => {
    // showRefresh defaults to false, so System and Users keep their own controls.
    assert.ok(read('src/components/AppHeader.tsx').includes('showRefresh = false'));
    for (const p of ['src/app/system/page.tsx', 'src/app/users/page.tsx']) {
      assert.ok(!read(p).includes('showRefresh'), `${p} must not opt in`);
    }
  });
});
